...
export async function GET(request: NextRequest) {
  ...
  if (!video_id)
    return new NextResponse(JSON.stringify({ error: "No video_id received." }), { status: 404 });
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ error: "Internal Server Error. Failed to fetch video." }),
      { status: 500 },
    );
  } finally {
    await cleanup();
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error(error);
    return new NextResponse(
      JSON.stringify({ error: "Internal Server Error. Failed to Add video." }),
      { status: 500 },
    );
  } finally {
    await cleanup();
...
export async function DELETE(request: NextRequest) {
  ...
  if (!video_id)
    return new NextResponse(JSON.stringify({ error: "No such video." }), { status: 404 });
  ...
  try {
    ...
    console.error(error);
    return new NextResponse(
      JSON.stringify({ error: "Internal Server Error. Failed to delete video." }),
      { status: 500 },
    );
  } finally {
    await cleanup();
