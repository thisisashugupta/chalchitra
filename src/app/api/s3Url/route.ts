...
export async function POST(req: Request) {
  return new NextResponse(JSON.stringify({ message: "Hello World!" }), {
    status: 200,
    headers: { message: "bro kya kr raha hai? GET request kar!" },
  });
}
...
export async function GET(req: Request) {
  try {
    ...
    return new NextResponse(JSON.stringify({ video_id, videoUrl, thumbnail_id, thumbnailUrl }), {
      status: 200,
    });
  } catch (error) {
    console.error(error);
    return new NextResponse(
      JSON.stringify({ error: "Internal Server Error. Failed to upload video." }),
      { status: 500 },
    );
  }
}
